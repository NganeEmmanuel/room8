...
public class VerificationController {
    ...
    public ResponseEntity<?> sendVerificationEmail(@Valid @RequestBody VerificationRequest request) {
    ...
    public ResponseEntity<?> verifyEmail(
        ...
        return verificationService.verifyEmailToken(email, token);
    }

    @PostMapping("/send-email")
    public ResponseEntity<?> sendEmail(@Valid @RequestBody SendEmailRequest request) {
        return verificationService.sendEmail(request);
    }

    @PostMapping("/send-sms-code")
    public ResponseEntity<?> sendSMSCode(@Valid @RequestBody SendSMSCodeRequest request) {
        return verificationService.sendSMSVerificationCode(request); //todo implement so that it just saves the code and the user id from the request ot redis. we will use twillo in the frontend to actually send the code
    }

    @PostMapping("/verify-sms-code")
    public ResponseEntity<?> verifySMSCode(@Valid @RequestBody VerifySMSCodeRequest request) {
        return verificationService.verifySMSCode(request);
    }

    @PostMapping("/verify-email-code")
    public ResponseEntity<?> verifyEmailCode(@Valid @RequestBody VerifyEmailCodeRequest request) {
        return verificationService.verifyEmailCode(request);
    }
