...
import { BathroomLocation, ListingStyle, ListingType } from '../../../constants/listingConstants.js';
...
const ListingForm = ({ initialData, onFormSubmit, pageTitle, submitButtonText }) => {
    ...
    const validateStep = () => {
        ...
        const requiredFields = {
            0: ['listingTitle', 'listingDescription', 'listingStyle', 'listingType'],
        ...
        // --- FIX: This validation now only runs if the Living Room section is visible ---
        if (currentStep === 2 && (listingData.listingType === 'Apartment' || listingData.listingType === 'Studio')) {
            if (listingData.hasLivingRoom) {
                // We only require the number, area is optional
                if (!listingData.numberOfLivingRooms) {
                    newErrors.numberOfLivingRooms = 'This field is required';
                }
            }
        }

        if (currentStep === 4 && (!listingData.images || listingData.images.length === 0) && imagePreviews.length === 0) {
    ...
    const listingCardPreviewProps = {
        ...
        // --- FIX: This now correctly displays the selected Listing Type ---
        roomType: ListingType[listingData.listingType] || 'N/A',
    ...
    return (
        ...
        {currentStep === 0 && (
            <section>
                ...
                <FormSelect label="Listing Type" name="listingType" value={listingData.listingType} onChange={handleChange} options={ListingType} required error={errors.listingType} />
        ...
        {currentStep === 2 && (
            <section>
                ...
                <FormToggle label="Shared Bathroom?" name="isSharedBathroom" checked={listingData.isSharedBathroom} onChange={() => handleToggleChange('isSharedBathroom')} />
                <FormToggle label="Shared Kitchen?" name="isSharedKitchen" checked={listingData.isSharedKitchen} onChange={() => handleToggleChange('isSharedKitchen')} />

                {(listingData.listingType === 'Apartment' || listingData.listingType === 'Studio') && (
                    <>
                        <h4 className="text-md font-medium text-gray-800 mt-6 border-t pt-4">Living Room Details</h4>
                        <FormToggle label="Includes a Living Room?" name="hasLivingRoom" checked={listingData.hasLivingRoom} onChange={() => handleToggleChange('hasLivingRoom')} />
                        {listingData.hasLivingRoom && (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 mt-4 bg-gray-50 p-4 rounded-md">
                                <FormInput label="Number of Living Rooms" name="numberOfLivingRooms" type="number" value={listingData.numberOfLivingRooms} onChange={handleChange} required min="1" error={errors.numberOfLivingRooms} />
                                <FormInput label="Living Room Area (sq m)" name="livingRoomArea" type="number" value={listingData.livingRoomArea} onChange={handleChange} placeholder="e.g., 20" min="0"/>
                            </div>
                        )}
                    </>
                )}
        ...
        {/* --- THIS IS THE NAVIGATION BUTTONS SECTION --- */}
